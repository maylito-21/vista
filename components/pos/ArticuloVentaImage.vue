<template>
  <div class="card p-0" @click="AgregarCarrito()">
    <div class="card-header mx-4 mt-2 p-1 text-center">
      <div class="avatar" v-if="articulo.image != null">
        <img :src="articulo.image.url" alt="image" class="border-radius-lg shadow" />
      </div>
    ...
    <div class="card-body pt-0 p-1 text-center">
      ...
      <div>
        <span class="badge " :class="['badge-success']"> STOCK {{ articulo.stock }}</span>
      </div>
      <hr class="horizontal dark my-2" />
      <h5 class="mb-0 text-sm">{{ Number(articulo.venta).toFixed(2) | currency('$') }}</h5>
      <!-- <p>{{articulo.stock}}</p> -->
